...
export const Week = ({ desde, hasta, data, setDateFilter }: Props) => {
  ...
  return (
    <div className="flex justify-start text-sm pb-2 mb-2">
      {data?.slice(desde, hasta).map((d, index) => (
        <span key={index} className="w-full flex justify-center items-center">
          {d.status === true && (
            <div className="bg-white text-[#2F2D53] w-9 h-9 rounded-full border-4 border-[#2F2D53] hover:text-white hover:bg-[#2F2D53] cursor-pointer">
              <div
                onClick={() => handleSetDay(d.day)}
                className="flex justify-center h-full text-center items-center"
              >
                {d.day}
              </div>
            </div>
          ...
          {data?.slice(desde, hasta).some((d) => d.status === true) &&
            d.status === false && (
              <div className="bg-white text-base-300 w-9 h-9 rounded-full border-4 border-base-300">
